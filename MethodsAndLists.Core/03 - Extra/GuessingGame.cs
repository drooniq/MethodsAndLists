...
namespace MethodsAndLists.Core
{
    public class GuessingGame
    {
        ...
        public int CorrectValueToGuess { get; set; }
        ...
        public GuessingGame(int correctValueToGuess, int maxGuesses)
        {
            if (maxGuesses <= 0 || correctValueToGuess <= 0)
                throw new ArgumentException();

            CorrectValueToGuess = correctValueToGuess;
            NumberOfGuesses = maxGuesses;
            GuessesLeft = maxGuesses;
        }

        public GuessResult Guess ( int valueToGuessFor )
        {
            if (valueToGuessFor < 0 )
            {
                throw new ArgumentException();
            }
            ...
            if (GuessesLeft == 0 && valueToGuessFor != CorrectValueToGuess)
            {
                return GuessResult.Fail;
            }

            if (valueToGuessFor>CorrectValueToGuess)
            {
                return GuessResult.Lower;
            } else if ( valueToGuessFor<CorrectValueToGuess)
            {
